	...
	public class ModelResourcesDesc
	{
		...
		private void InitializeTypeIdsInInsertOrder()
		{
			...
			*/
            ...
            // ovde dodaješ REDOSLED svih konkretnih klasa iz modela
            // tako da svaki “target” (klasa B) dolazi pre klase A koja ga referencira

            // 1) BAZNE IDENTIFIEDOBJECT–LIKE KLASE
            typeIdsInInsertOrder.Add(ModelCode.IDOBJ);      // IdentifiedObject
            typeIdsInInsertOrder.Add(ModelCode.PSR);        // PowerSystemResource
            typeIdsInInsertOrder.Add(ModelCode.EQUIPMENT);  // Equipment
            typeIdsInInsertOrder.Add(ModelCode.CONDEQIP);   // ConductingEquipment

            // 2) SCHEDULE HIJERARHIJA
            // BasicIntervalSchedule → IrregularIntervalSchedule → IrregularTimePoint
            typeIdsInInsertOrder.Add(ModelCode.BSCINTSCHEDULE);
            typeIdsInInsertOrder.Add(ModelCode.IISCHEDULE);
            ...
            // 4) OUTAGE HIJERARHIJA
            // OutageSchedule referencira IISCHEDULE
            typeIdsInInsertOrder.Add(ModelCode.OUTAGESCH);

            // 5) SWITCHING OPERATION referencira OutageSchedule i Switch
            typeIdsInInsertOrder.Add(ModelCode.SO);

            // 6) SWITCH HIJERARHIJA
            // Switch → GroundDisconnector
            typeIdsInInsertOrder.Add(ModelCode.SWITCH);
            typeIdsInInsertOrder.Add(ModelCode.GNDDISC);


            // 87) PO potrebi - drugi konkretni tipovi
            // npr. TransformerWinding, WindingTest, Terminal, …
            // (u zavisnosti šta još imaš u profilu)
